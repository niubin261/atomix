...
public class WebSocketKeepAliveRequest extends KeepAliveRequest implements WebSocketRequest<WebSocketKeepAliveRequest> {
  ...
  public static class Builder extends KeepAliveRequest.Builder {
    ...
    public KeepAliveRequest copy(KeepAliveRequest request) {
      return new io.atomix.copycat.protocol.websocket.request.WebSocketKeepAliveRequest(id, request.session(), request.commandSequence(), request.eventIndex());
    }

    @Override
    public KeepAliveRequest build() {
      return new io.atomix.copycat.protocol.websocket.request.WebSocketKeepAliveRequest(id, session, commandSequence, eventIndex);
    }
  }
}
