...
import io.atomix.protocol.websocket.request.WebSocketRequest;
import io.atomix.protocol.websocket.response.WebSocketResponse;
...
public abstract class WebSocketConnection<T extends WebSocketBase> implements ProtocolConnection {
  protected T socket;
  protected final ObjectMapper textMapper = new ObjectMapper();
  ...
  /**
   * Reads the given value using the JSON serializer.
   */
  protected <T> T readJson(byte[] bytes, Class<T> type) {
    try {
      return textMapper.readValue(bytes, type);
    } catch (IOException e) {
   ...
   */
  protected byte[] writeJson(Object object) {
    try {
      return textMapper.writeValueAsBytes(object);
    } catch (IOException e) {
      throw new RuntimeException(e);
    }
  }
