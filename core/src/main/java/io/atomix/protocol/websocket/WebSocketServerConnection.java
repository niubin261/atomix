...
import io.atomix.protocol.websocket.request.WebSocketPublishRequest;
import io.atomix.protocol.websocket.request.WebSocketRequest;
import io.atomix.protocol.websocket.response.*;
...
public class WebSocketServerConnection extends WebSocketConnection<ServerWebSocket> implements ProtocolServerConnection {
  private static final Logger LOGGER = LoggerFactory.getLogger(io.atomix.copycat.protocol.websocket.WebSocketServerConnection.class);
  ...
  protected void onRequest(WebSocketRequest request) {
    if (request.type() == WebSocketRequest.Type.CONNECT) {
      ...
      WebSocketCommand command = readJson(commandRequest.bytes() WebSocketCommand.class);
