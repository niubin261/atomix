...
public class WebSocketPublishRequest extends PublishRequest implements WebSocketRequest<WebSocketPublishRequest> {
  ...
  public static class Builder extends PublishRequest.Builder {
    ...
    public PublishRequest copy(PublishRequest request) {
      return new io.atomix.copycat.protocol.websocket.request.WebSocketPublishRequest(id, request.session(), request.eventIndex(), request.previousIndex(), request.events());
    }

    @Override
    public PublishRequest build() {
      return new io.atomix.copycat.protocol.websocket.request.WebSocketPublishRequest(id, session, eventIndex, previousIndex, events);
    }
  }
}
