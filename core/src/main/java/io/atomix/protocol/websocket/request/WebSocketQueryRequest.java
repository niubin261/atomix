...
public class WebSocketQueryRequest extends QueryRequest implements WebSocketRequest<WebSocketQueryRequest> {
  ...
  public static class Builder extends QueryRequest.Builder {
    ...
    public QueryRequest copy(QueryRequest request) {
      return new io.atomix.copycat.protocol.websocket.request.WebSocketQueryRequest(id, request.session(), request.sequence(), request.index(), request.bytes(), request.consistency());
    }

    @Override
    public QueryRequest build() {
      return new io.atomix.copycat.protocol.websocket.request.WebSocketQueryRequest(id, session, sequence, index, bytes, consistency);
    }
  }
}
